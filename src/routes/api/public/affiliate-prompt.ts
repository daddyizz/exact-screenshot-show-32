...
async function handle(request: Request) {
  ...
  const body = `(function(){
    var CFG=${js({
      origin,
      delay: Number(settings.delay_seconds ?? 8),
      snooze: Number(settings.close_snooze_minutes ?? 30),
      cooldown: Number(settings.clicked_cooldown_hours ?? 24),
      blogId,
      links,
    })};
    ...
    function choose(){var h=norm(document.title+' '+((document.body&&document.body.innerText)||'')),best=null,bestScore=0;CFG.links.forEach(function(l){var score=0;String(l.keywords||'').split(',').forEach(function(k){if(has(h,k))score+=l.link_type==='product'?4:2});if(l.category&&has(h,l.category))score+=l.link_type==='product'?2:3;if(score>0&&(best===null||score>bestScore||(score===bestScore&&Number(l.priority||100)<Number(best.priority||100)))){best=l;bestScore=score}});return best}
