...
function Dashboard() {
  ...
  const handleAddBlogClick = () => {
    ...
    if (planUsage.isError || !planUsage.data) {
      toast.error("Could not verify your blog limit. Please try again.");
      return;
    }
  ...
  const runNow = useMutation({
    mutationFn: (blogId: string) => runNowFn({ data: { blogId, origin: window.location.origin } }),
    onSuccess: async (result) => {
      toast.success(result.status === "published" ? `Published: ${result.detail}` : result.status === "drafted" ? `Draft ready: ${result.detail}` : result.detail);
      await queryClient.invalidateQueries({ queryKey: ["blogs"] }); await queryClient.invalidateQueries({ queryKey: ["post-counts"] }); await queryClient.invalidateQueries({ queryKey: ["posts"] }); await queryClient.invalidateQueries({ queryKey: ["my-plan-usage"] });
    },
    onError: (error: Error) => toast.error(error.message),
  });
  ...
  return <div className="space-y-8">
    ...
    {blogs.isLoading ? <p className="text-sm text-muted-foreground">Loading blogs…</p> : (blogs.data?.length ?? 0) === 0 ? <div className="surface-panel p-10 text-center"><Sparkles className="mx-auto size-6 text-primary" aria-hidden /><h2 className="mt-4 text-xl font-semibold">No blogs yet</h2><p className="mx-auto mt-2 max-w-sm text-sm text-muted-foreground">Add your first blog, pick a niche and BlogPilot will help you build a topic plan.</p></div> : <div className="grid gap-4 md:grid-cols-2">{blogs.data?.map((blog) => <div key={blog.id} className="surface-panel p-5">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        ...
        <Button size="sm" variant="secondary" onClick={() => runNow.mutate(blog.id)} disabled={runNow.isPending}><PlayCircle aria-hidden />{runNow.isPending && runNow.variables === blog.id ? "Running…" : "Run autopilot now"}</Button>
      </div>
    </div>)}</div>}
