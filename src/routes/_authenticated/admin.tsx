...
function AdminPage() {
  ...
  if (adminCheck.isError) {
    return (
      <ErrorPanel
        ...
        message={(adminCheck.error as Error).message}
        onRetry={() => void adminCheck.refetch()}
      />
    );
  }
    ...
    return (
      <div className="surface-panel p-10 text-center">
        ...
        <p className="mx-auto mt-2 max-w-sm text-sm text-muted-foreground">
          This signed-in account does not currently have the admin role.
        </p>
      </div>
    );
  }
  ...
  if (overview.isError) {
    return (
      <ErrorPanel
        title="Admin access confirmed, but the console failed to load"
    ...
    <div className="space-y-7">
        ...
        <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Send an invitation and choose the starting plan and role.</DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
              <Button
                ...
                disabled={inviteMutation.isPending || !invite.email || !invite.displayName}
              >
      ...
      <Tabs defaultValue="users" className="space-y-5">
        ...
        <TabsContent value="billing">
          <div className="surface-panel divide-y divide-border">
            {users.map((u) => (
              <div key={u.id} className="grid gap-2 p-4 sm:grid-cols-6 sm:items-center">
                <div className="sm:col-span-2">
                  <p className="font-medium">{u.displayName}</p>
                  <p className="text-xs text-muted-foreground">{u.email}</p>
                </div>
                <PlanBadge plan={u.plan} />
                <span className="text-sm capitalize">{u.billingProvider}</span>
                <span className="text-sm">{u.usage.aiDrafts}{u.plan === "free" ? " / 5 drafts" : " drafts"}</span>
                <span className="text-sm capitalize">{u.subscriptionStatus}</span>
              </div>
            ))}
          </div>
        </TabsContent>
      ...
      <Dialog open={Boolean(editUser)} onOpenChange={(open) => !open && setEditUser(null)}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            {editUser && (
              <div className="flex flex-wrap gap-2">
                <Button type="button" variant="outline" onClick={() => roleMutation.mutate({ userId: editUser.id, role: "admin", grant: !editUser.roles.includes("admin") })}>
                  {editUser.roles.includes("admin") ? "Remove admin" : "Make admin"}
                </Button>
                <Button type="button" variant="outline" onClick={() => roleMutation.mutate({ userId: editUser.id, role: "moderator", grant: !editUser.roles.includes("moderator") })}>
                  {editUser.roles.includes("moderator") ? "Remove moderator" : "Make moderator"}
                </Button>
              </div>
            )}
          </div>
