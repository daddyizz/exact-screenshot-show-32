...
function QueuePage() {
  ...
  const blogs = useQuery({ queryKey: ["blogs"], queryFn: async () => { const { data, error } = await supabase.from("blogs").select("id, name").order("created_at", { ascending: false }); if (error) throw error; return data; } });
  ...
  const makeImage = useMutation({ mutationFn: (id: string) => imageFn({ data: { postId: id } }), onSuccess: async () => { toast.success("Featured image ready"); await queryClient.invalidateQueries({ queryKey: ["posts", blogId] }); }, onError: (error: Error) => toast.error(error.message) });
