...
export const runAutopilotNow = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await writeAutopilotRun(supabaseAdmin, { userId: context.userId, blogId: blog.id, postId: outcome.postId ?? null, triggerSource: "manual", status: outcome.status, detail: outcome.detail, publishedUrl: outcome.url, startedAt });
...
export const runDueAutopilot = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const blog of due) {
      ...
      await supabaseAdmin.from("blogs").update({ autopilot_last_run_at: new Date().toISOString() }).eq("id", blog.id);
      await writeAutopilotRun(supabaseAdmin, { userId: context.userId, blogId: blog.id, postId: outcome.postId ?? null, triggerSource: "dashboard", status: outcome.status, detail: outcome.detail, publishedUrl: outcome.url, startedAt });
    }
