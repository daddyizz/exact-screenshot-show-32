...
create table if not exists public.autopilot_runs (
  ...
  trigger_source text not null default 'scheduled'
    check (trigger_source in ('scheduled','manual','dashboard')),
  status text not null
    check (status in ('skipped','drafted','published','error')),
  detail text,
  published_url text,
...
create index if not exists autopilot_runs_created_at_idx
  on public.autopilot_runs(created_at desc);

create index if not exists autopilot_runs_blog_id_idx
  on public.autopilot_runs(blog_id, created_at desc);

create index if not exists autopilot_runs_user_id_idx
...
drop policy if exists "Users can read own autopilot runs"
